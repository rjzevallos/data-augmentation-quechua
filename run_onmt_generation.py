...
parser.add_argument("-t", "--task", type=str, default='quz_labeled', help="choose task: quz_labeled, quz_baseline")
...
def generate_data(task_name, pair_mode='diverse_connect', append_index=True, no_clustering=False, filter_rate=0.5):
    ...
    onmt_data_path = CONFIG['path']['OnmtData'] + 'Data/'
    raw_data_path = CONFIG['path']['ClusteringResult']
    if no_clustering:
        # all_file = list(filter(lambda x: '.json' in x and '_nc' in x, os.listdir(raw_data_path)))
        all_file = list(filter(lambda x: '.json' in x and '_nc' in x and 'quechua' in x, os.listdir(raw_data_path)))
    else:
        # all_file = list(filter(lambda x: '.json' in x and '_nc' not in x, os.listdir(raw_data_path)))
        all_file = list(filter(lambda x: '.json' in x and '_nc' not in x and 'quechua' in x, os.listdir(raw_data_path)))
